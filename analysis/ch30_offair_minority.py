...
mixture-model reclamation route of Sec. 5.2 applies:
  ...
  - split at F = 2 null_power_ratio (the gap 1.5--10 x null_power_ratio is essentially empty);
...
null_power_ratio = float(study[30]["null_power_ratio_analytic"])
IDEAL_WIDTH = 2.39e-3          # sqrt(1/R + 1/2R), R = 262144 (Sec. 5.2)
SPLIT = 2.0                    # in units of null_power_ratio; the 1.5--10 gap is empty
...
gap = ((F >= 1.5) & (F < 10)).sum()
low = F < SPLIT
print(f"ch30: n_valid={n}  null_power_ratio_analytic={null_power_ratio:.6f}")
print(f"  high clump (10--14 x null_power_ratio): {hi_clump} ({100*hi_clump/n:.2f}%)")
print(f"  gap (1.5--10 x null_power_ratio):       {gap} ({100*gap/n:.2f}%)")
print(f"  minority (F < {SPLIT:.0f} null_power_ratio):     {int(low.sum())} "
      f"({100*low.sum()/n:.2f}%)")
...
for u in low_units:
    ...
    rows.append({"unit": int(u), "date": d, "n_frames": int(x.size),
                 "mean_over_null_power_ratio": f"{x.mean():.5f}",
                 "std_over_null_power_ratio": (f"{x.std(ddof=1):.5f}" if x.size >= 2
                                  else "")})
    means.append(x.mean())
    if x.size >= 3:
