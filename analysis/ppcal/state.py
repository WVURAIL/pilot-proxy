# coding=utf-8
"""Assemble the full per-channel calibration state once, for every consumer.

One pass builds eras, the calibrated ``mu``, the threshold ladder on that
``mu``, the BAO-priced threshold read back from ``eta_bao.csv`` if it has
been produced, and the resulting disposition.  Both figure scripts and the
table writer consume this so no two of them can disagree.
"""
...
from .calib import calibrate, CLEAN_OCCUPANCY, EXCISE_OCCUPANCY
...
CARRIER_DOMINATED_DB = 3.0
ETA_WORKING = 1.4          # global fallback when no per-channel eta is priced
...
class ChannelState:
    ...
    def eta_channel(self):
        """The channel's own BAO-priced threshold, or the global fallback."""
        v = self.bao.get("eta_cost_cap") if self.bao else None
    ...
    def occ_global(self):
        ...
        return self.cal.occupancy["%g" % ETA_WORKING]
...
def decide(cal, bao, eta, occ):
    """Keep/excise from the calibrated null, with the residual bound alongside.
    ...
    *not certified*, not that it is exceeded -- a measured coherence time can
    only move the residual down. No verdict below depends on it: excision is
    decided on carrier dominance and occupancy, both of which are coherence
    free.
    """
    carrier = cal.mu_shift_db > CARRIER_DOMINATED_DB
    bracket = ""
    if bao and bao.get("r_cost_cap") and bao.get("r_tol_dilation"):
        mult = bao["r_cost_cap"] / bao["r_tol_dilation"]
        at_cap = not bool(bao.get("tau_measured"))
    ...
    if occ >= EXCISE_OCCUPANCY:
        return ("excise", "excise",
                "%.1f%% of latest-era frames sit above F > %.3f mu%s"
                % (100 * occ, eta, bracket))
    tier = "light" if occ <= CLEAN_OCCUPANCY else "heavy"
...
def build(products, bao_csv=None):
    """Every channel's state, ordered by physical channel."""
    bao_all = _read_bao(bao_csv)
    out = []
    for c in sorted(load_all(products), key=lambda c: c.ch):
        ...
        blind = calibrate(c, np.ones(c.fstat.size, bool), "full archive",
                          c.n_units_raw)
        bao = bao_all.get(c.ch, {})
        s = ChannelState(
            c=c, segs=segs, fmask=fmask, umask=umask, cal=cal, blind=blind,
            bao=bao, carrier_dominated=cal.mu_shift_db > CARRIER_DOMINATED_DB,
            verdict="", disposition="", reason="")
        eta = s.eta_channel
        s.verdict, s.disposition, s.reason = decide(cal, bao, eta,
                                                    s.occ_at(eta))
        out.append(s)
    return out
