...
`sibling-probe-ch32` survey inventory is not in this repository. Restore that
...
### Amendment A1 (2026-07-29): baseband frame correction and detector v2

Registered as an amendment under the rule above, after the first capped
production pass and before any relaunch.

1. **Frame correction.** The deployed weight ROM assumed the coarse-channel
   center at Nyquist in the channelized stream; measurement on CHIME/FRB
   baseband raw (event 68399317, freq_id 844) and on the production scan's
   ...
   0.3.0.dev0; kernel core 2.0.0; product schema
   `pilotproxy_detector_datatrawl_v3`. Per the detector_version invariant,
   these bumps reopen the K = 128 resolution decision; we retain K = 128 on
   the same evidence (offset survey) unless the redone scan contradicts it.

2. **Mistuned-epoch product labeling.** All `chime-pilots` products written
   with the legacy bank (weights archived under `weights/legacy_halfband/`)
   are labeled the *legacy-halfband epoch*. They remain valid for: the
   transmitter offset survey and per-channel retune targets (their
   `integrated_spectrum_*` arrays are tuning-independent), the PFB band-edge
   bias-floor characterization (measured -0.42% mean excess, predicted
   -0.413% from the response convolution), and odd/even frame-parity checks.
   They must not be used for pilot detection, masking-depth, or
   scaling-class claims. Detection-bearing claims come only from products
   whose `detector_version` embeds kernel >= 2.0.0 and the v2 profile hash.

3. **Fine-statistic decision rule (pre-registration).** The v2 fine
   reduction ships with its thresholding policy fixed in code and recorded
   in `detector_contract_json.fine_reduction`: null-bulk calibration on the
   independent non-designated bins (median plus left-side scale at fixed
   P_FA = 1e-3, guard of 1 fine bin, census exclusions), lower-quantile
   fallback on contaminated bulks, any-bin detection, and the
   non-designated flag rate reported as the live false-alarm monitor. The
   exact int64 all-bin marginal identity against the v1 statistic is
   enforced per frame at run time.

4. **Per-line excess calibration note.** With nominal-tuned templates, a
   transmitter offset by ~1.8-2.2 fine bins leaks into the +-2-bin
   references at approximately -21 dB, capping that line's own F even when
   its target-spectrum power is large (measured on the event file: the
   -517 Hz line reaches S_t ~ 850x floor but F ~ 124). Detection is
   unaffected; any per-line excess-to-INR statement in the paper must apply
   the response-matrix correction rather than reading F at the line
   directly. The v1 all-bin ledger is unaffected (Parseval-exact).
