...
| 12 | Coherent before/after: bearing-steered beamformed spectrum, all-frames vs mask-kept accumulators | visibility analyzer (planned) over a bounded companion scan | companion-scan products + gains companion | design complete: geometry from the CHIME Overview (arXiv:2201.07869; 305 mm pitch + published 0.071 deg rotation), feed axis in file order with the serial-to-position mapping a declared input, gains external to the baseband event datasets (optional input; the before/after contrast is gain-invariant). Analyzer pending build |
| 13 | Mask expansion across one full ATSC sibling span: 8-VSB shelf before/after the pilot-keyed mask | same analyzer, `--mask-from <pilot product>` | same companion scan | cleared 2026-07-08: 1,028-event probe of the ch32 span shows mean 13.3/17 sibling channels archived (78%); per-channel gaps are node-uptime epochs (count-quantized at 973/168/354/114) and self-correct under intersection selection with the pilot products. Evidence: data/sibling-probe-ch32 inventory (retain). Sizing: ~4k files per span |
