...
atomic rename. Thus, a visible `_per_pilot/<freq_id>.npz` is a complete
checkpoint.
...
- The DTV 18 and DTV 20 zero-point checks in item 1 can use a snapshot of the
  survey checkpoints. Copy `_per_pilot/<freq_id>.npz` into `snapshot/`, then run:
...
2. **Choose three `freq_id` values** from the transmitter census in the calibration
   ...
   - **Control:** use a normal DTV pilot profile whose nominal ATSC pilot is
     ...
     selection, not a propagation prediction. Match the census epoch to the
     data epoch, and note that the shipped census lists at least one station
     on every physical channel 14-36, so a strictly station-free control may
     not exist; in that case use the most isolated channel and take the zero
     point from the empirical calibration (see the runbook's H0 section).
     Do not select an arbitrary
