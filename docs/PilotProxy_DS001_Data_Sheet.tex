...
to \ppFineBinHz\ Hz. The fine path is the deployed decision path: coherent
window integration recovers up to \ppCoherentGainDb~dB of deflection
sensitivity over the all-incoherent reduction, and the designated-set CFAR
forms the per-frame mask bit, gated on a deterministic (bit-exact
host/device) FFT implementation. The coarse path is the survey's recorded
$F>\mu_0$ flag and the bit-exact marginal check of the fine path's inputs
...
The frame size is not a free detector parameter: the adapter matches the correlator's frame, and the kernel requires only that the matched value be a positive multiple of \(K=128\). The checked-in \code{chime_dtv_fengine} receiver profile uses \(2^{14}=16{,}384\) channelized samples per input stream --- the CHIME F-engine upgrade frame this integration is designed for --- and declares the frame-verified DC-centered baseband convention (\field{baseband_frame.channel_center_normalized}=0.0), validated against archived CHIME baseband; the earlier half-band bank is archived under \code{weights/legacy_halfband/} and must not be deployed. For this profile and one selected coarse channel, \(\ppChimeStreams\times\ppWindowsPerStream=\ppChimeRowsPerFrame\) detector rows form one frame. The 16,384-sample frame is the planned CHIME F-engine upgrade interface used by the repository's analysis and tests; it is not a claim about the currently deployed correlator frame. The software does not infer one universal value from HDF5 metadata: \code{chime-run} accepts an explicit per-run frame-size override, while \code{chime-scan} takes \code{nfft} from the datatrawl instrument configuration. Operators must verify and record the resolved value for each real-data product.
...
regeneration alone, with no kernel or schema change. Coarse-only deployed
operating points were nonetheless evaluated on measured ROCs and set
aside: the $\mu_0$ point spends about half of verified-quiet time, fixed
false-alarm points leak weak channels, and an ROC-derived point such as
the Youden maximum assumes equal costs, which masking does not have. The
adopted deployed decision is instead the fine designated-set CFAR
computed in the kernel, which holds the fixed false-alarm policy on the
fine axis (\code{docs/DESIGN_DECISIONS.md}). Because \field{coarse_power_ratio}
and the fine spectrum are persisted per frame, previously recorded epochs
can be re-scored offline under any such rule.

CHIME per-pilot archive products use
\field{schema_version}=\texttt{\ppSchemaVThree}: the complete shared array
...
defines the shared arrays and \code{docs/PRODUCT_SCHEMA.md} the
fine-reduction fields and their enforced invariants. The shipped weight bank and manifest are
...
\begin{itemize}
  ...
  \item detector-bin spacing: \(390.625\,\mathrm{kHz}/128=\ppDetectorBinHz\,\mathrm{Hz}\);
