...
\title{PilotProxy CUDA F-Statistic DTV Pilot Detector\\Product Specification}
\author{West Virginia University}
\date{PilotProxy-DS001 \\ August 2026}
...
\begin{minipage}[t]{0.39\textwidth}
  ...
  {\small PilotProxy-DS001, August 2026}
\end{minipage}
\hfill
\begin{minipage}[t]{0.58\textwidth}
  \raggedleft
  {\fontsize{18}{20}\selectfont\bfseries CUDA F-Statistic DTV Pilot Detector}\par
...
\begin{itemize}
  ...
  \item Compile-time 64- or 128-sample detector rows using packed complex
        4+4 bit input; the shipped CHIME bank selects 128.
  ...
  \item Time-coherent front end: \code{int32} row sums, padded fine
        spectrum, any-bin CFAR detection.
...
Kernel outputs & \code{mask} (deployment; 1 = reject), \code{uint64} powers, \code{int32} row sums \\ \hline
...
Synthetic threshold & \code{threshold_data_shelf_snr_db}; converted by evaluator host software \\ \hline
...
\vfill
{\small\color{FStatGray}Synthetic thresholds are distinct from the current CHIME $F>\mu_0$ mask; $F>1$ is the basic raw convention. Measure performance on the target system.}
...
\caption{Proposed deployment system context. The detector stage consumes the
same channelized baseband as the correlator and enqueues one binary rejection
decision per aligned frame; the correlator pops the queue when its own
processing of that frame completes, so the decision is synced to the
integration it masks. The detector stage, runtime bundle, and mask API are
implemented and tested; the queue and correlator synchronization are the
proposed Kotekan interface (\code{docs/KOTEKAN_INTERFACE_PREP.md}), and the
producer/consumer rate margin was measured on an earlier kernel build and
must be re-measured for the current core.}
...
all rows, the windows are integrated coherently and only the feed sum
remains incoherent, recovering up to \ppCoherentGainDb~dB of deflection
sensitivity for \(W=128\) windows (\(10\log_{10}\sqrt{W}\), less
scalloping). For each weight term
\(n\in\{t,r1,r2\}\) and detector row \(m\), it returns the complex dot
product
\begin{equation}
  z[n,m]=\sum_{k=0}^{K-1} w_n[k]\,x_m[k]
  \qquad(\text{exact \code{int32} components}).
\end{equation}
Host software reshapes each term's rows to
\([N_{\mathrm{streams}},\,W]\) with \(W=\mathrm{frame\_size\_samples}/K\)
windows per stream, applies an FFT along the window axis with
\(\times\ppFinePadFactor\) zero padding, and sums incoherently over streams:
\begin{equation}
...
coherent in time within one frame and incoherent across streams and frames.
No inter-window phase corrector is required: each term's steering frequency
is an integer bin of the \(K\)-sample window, so an offset tone's
window-to-window phase survives stage 1 exactly
(\(e^{2\pi i c_n j}=1\) for integer bin \(c_n\)). The omitted
\emph{intra-window} fine rotation \(e^{-2\pi i kb/(\mathrm{pad}\cdot WK)}\)
is the window capture factor documented in \code{docs/METHOD_SPEC.md}
(\(\mathrm{sinc}^2\) scalloping: \ppCaptureLossThreeHundredDb~dB at
300\,Hz, at most \ppCaptureLossEdgeDb~dB at the fine-span edge). Two
identities are enforced at run time, not assumed: the exact int64
marginal identity, \(\sum_m|z[n,m]|^2\) reproduces the
\code{FStat_Compute_Powers_U64} output bit-for-bit; and the float Parseval
identity, \(\sum_b S[n,b]=\mathrm{pad}\cdot W\cdot P_n\), gated at ULP
tolerance. \(F\) therefore remains the exact all-bin marginal of the fine
spectrum. Fine-bin detection uses a pre-registered null-bulk CFAR over the
independent non-designated bins with an any-bin decision rule; the stored
fields, CFAR calibration modes, and detection-list layout are specified in
\code{docs/PRODUCT_SCHEMA.md}.

\subsection{Distributional Statement ($H_0$)}
\label{ssec:dist}

Under the null hypothesis --- no pilot, a locally flat spectrum across the
three terms, and circular complex Gaussian channelized noise --- each
term's dot product \(z_n\) is complex Gaussian, so each per-row power
\(|z_n|^2\) is a scaled \(\chi^2_2\) variable, and the three terms are
independent because they sample disjoint detector bins. For one detector
row with equal term norms, write \(|z_t|^2 = \tfrac{s}{2}U\) and
...
\;\sim\; F(2,4),
\end{equation}
the F distribution with \((2,4)\) degrees of freedom: a 2-dof power over
the mean of two independent 2-dof powers. The name F-statistic is
therefore literal, not borrowed.

The deployed statistic sums powers over \(M\) rows before the ratio
(\code{all_rows_summed_before_ratio}), which replaces \((2,4)\) by
\((2M,\,4M)\): under the same assumptions \(F \sim F(2M, 4M)\). Two
location facts prevent a common misreading. The per-row \(F(2,4)\) has
mean \(4/(4-2)=2\) and a heavy right tail; the frame statistic with
\(M=\ppChimeRowsPerFrame\) rows concentrates tightly, with
\(\mathrm{E}[F(2M,4M)] = 4M/(4M-2) \to 1\). The mask's zero point
\(\mu_0\) is the ratio of expectations \(2 n_t/(n_{r1}+n_{r2})\) evaluated
with the exact quantized norms --- the value the concentrated frame
statistic approaches --- and the mask itself relies on no distributional
form at all: it is an exact integer comparison against \(\mu_0\).

Caveats, stated rather than assumed: with unequal reference norms, the
denominator is a weighted sum of two \(\chi^2_2\) terms and the \(4M\)-dof
form is a Satterthwaite-style approximation; correlated or non-Gaussian
interference violates the row-independence assumption. Each fine bin
repeats the construction across \(N_{\mathrm{s}}\) streams,
\(F_2[b] \sim F(2N_{\mathrm{s}},\,4N_{\mathrm{s}})\) on independent bins
under \(H_0\); the fine CFAR deliberately calibrates its null from the
data (the null bulk) rather than assuming this shape.

\begin{figure}[H]
\centering
\input{figures/detector_datapath.tikz}
\caption{Detector datapath in the filterbank view. Stage 1 is a critically
sampled DFT filterbank with a rectangular (one-tap) prototype, evaluated
only at the three needed channels through the int4 matched weights; the
window-axis FFT is the second stage of a standard two-stage zoom
channelization, refining the selected detector bin from \ppDetectorBinHz\ Hz
to \ppFineBinHz\ Hz. The fine path is an implemented, inactive decision
candidate: coherent window integration recovers up to
\ppCoherentGainDb~dB of deflection sensitivity over the all-incoherent
reduction, and the designated-set CFAR can form a per-frame candidate mask bit
using a deterministic (bit-exact host/device) FFT implementation. The coarse
path supplies the current archive $F>\mu_0$ rejection flag and the bit-exact
marginal check of the fine path's inputs
...
For synthetic threshold tests, the user supplies \code{threshold_data_shelf_snr_db}. Host software converts this shelf-SNR threshold to the raw F-statistic coordinate and then to the rational half-threshold used by the kernel:
...
The first index is the standalone ascending-frequency reference coordinate. The second is the post-spectral-sense-normalization coordinate stored in the shipped CHIME manifest and printed by \code{pilot-proxy list-channels}. The nominal references are two detector bins below and above the target. In the shipped DC-centered bank, physical channel 14 is the only adaptive case: its target lies near a coarse-channel boundary, so the generated layout wraps the lower reference across the circular coarse-channel spectrum while preserving the two-bin offset, and \code{list-channels} prints the corresponding placement \code{NOTE}. No shipped channel collides with, or shifts around, the forbidden coarse-channel DC tone. Detection and validation reports copy the selected manifest row into \field{selected_weight_layout}, so this placement remains visible in each result.
...
\begin{itemize}
  \item \code{DetectorCoreProfile}: locked kernel contract, including the
        selected \(K\in\{64,128\}\), \(N=3\), int4 packed input, uint64
        powers, and rational half-threshold input.
  \item \code{ReceiverProfile}: RF band, channelizer geometry, spectral sense, baseband frame convention (\field{baseband_frame.channel_center_normalized}), frame size, input-stream count, quantization defaults, detector-bin ENBW, and pilot-capture efficiency.
...
The frame size is not a free detector parameter: the adapter matches the correlator's frame, and the kernel requires that value to be a positive multiple of the profile-selected \(K\). The checked-in \code{chime_dtv_fengine} receiver profile selects \(K=\ppDetectorWindowSamples\) and uses \(2^{14}=16{,}384\) channelized samples per input stream --- the CHIME F-engine upgrade frame this integration is designed for --- and declares the frame-verified DC-centered baseband convention (\field{baseband_frame.channel_center_normalized}=0.0), validated against archived CHIME baseband; the earlier half-band bank is archived under \code{weights/legacy_halfband/} and must not be deployed. For this profile and one selected coarse channel, \(\ppChimeStreams\times\ppWindowsPerStream=\ppChimeRowsPerFrame\) detector rows form one frame. The CHORD profiles instead select \(K=64\) with an 8,192-sample frame; both profile families retain 128 windows per stream for the frozen fine transform. The frame sizes are integration contracts, not claims about a universally deployed correlator frame. The software does not infer one universal value from HDF5 metadata: \code{chime-run} accepts an explicit per-run frame-size override, while \code{chime-scan} takes \code{nfft} from the datatrawl instrument configuration. Operators must verify and record the resolved value for each real-data product.

\section{Result Schema and Mask Convention}

Detector and synthetic SNR-evaluator JSON outputs include a versioned \field{result_schema} object with \code{schema_version=pilotproxy_result_schema_v1}. This object records the statistic definitions, layout metadata, calibration constants, threshold fields, fixed-point contract, and mask convention needed to interpret those results. Audit, quantization, summary, injection, and CHIME products use their own documented top-level schemas rather than this object.
...
\end{equation}
The basic uncorrected convention would call \(F>1\) positive excess. The current implementation instead masks a valid CHIME frame only when \(F>\mu_0\), evaluated by integer cross multiplication. Independent recomputation over the shipped manifest gives \(\mu_0\in[\ppMuZeroMin,\,\ppMuZeroMax]\) across the \ppWeightChannelCount\ channels. This correction accounts for unequal norms introduced by int4 weight quantization; it is not an empirical RFI threshold and does not establish an operational false-positive rate.

The operating point itself is carried as per-channel rational data in the
runtime bundle (\field{positive_excess_half_threshold_num},
\field{positive_excess_half_threshold_den}); the kernel comparison is
threshold-agnostic, so recalibrating the recorded rule is a bundle
regeneration alone, with no kernel or schema change. The active archive
decision remains the norm-corrected coarse positive-excess comparison above.
The implemented fine designated-set CFAR is a candidate whose calibration
status is \code{pending_campaign}; its spectrum, threshold, and exceedances are
diagnostic and do not set \field{reject_mask}. Because
\field{coarse_power_ratio} and the fine spectrum are persisted per frame,
previously recorded epochs can be re-scored offline after a separately
versioned policy is calibrated and adopted.

CHIME per-pilot archive products use
\field{schema_version}=\texttt{\ppPerPilotSchema}: the complete shared array
set plus the fine-reduction fields (fine spectrum, CFAR calibration columns,
and the ragged any-bin detection list). \code{docs/PER_PILOT_PRODUCT_FIELDS.md}
defines the shared arrays, \code{docs/FINE_REDUCTION_PRODUCTS.md} defines the
fine-reduction fields, and \code{docs/PRODUCT_SCHEMA.md} defines their active,
diagnostic, and candidate decision roles. The shipped weight bank and manifest are
identified by SHA-256 prefixes \texttt{\ppWeightBankShaShort} and
...
component by 128 per tap. Therefore, the largest supported window, 128 taps,
has an absolute component bound of 16,384 and fits in signed 16-bit storage; a
256-sample window could reach 32,768 and exceed the positive signed-int16
limit. Under the same bound, one magnitude-squared term fits in \code{uint32},
and the configured block power sum fits in \code{uint64}.
...
\apicode{FStat_Compute_RowSums_I32} & Exact \code{int32} complex per-row dot products (term-major, stream-major rows, interleaved re/im); the int64 sum of \(|z|^2\) over rows reproduces \apicode{FStat_Compute_Powers_U64} exactly.\\ \hline
\apicode{FStat_Supports_RowSums} & Runtime capability probe; returns 1 when the loaded library provides the row-sum entry.\\ \hline
\apicode{FStat_Compute_FinePowers_U64} & On-device fine-reduction power stage (core 2.1.0): frozen fxfft256 v1 fixed-point FFT over the row-sum buffer plus exact \code{uint64} feed-power sums per fine bin; handle-free, composes directly with \apicode{FStat_Compute_RowSums_I32} output; bit-reproducible for any launch schedule. Policy-free: the fine statistic, CFAR, and designated-set decision form downstream.\\ \hline
\apicode{FStat_Supports_FinePowers} / \apicode{FStat_GetFineSpecs} & Fine-stage capability probe and frozen geometry query (128 windows, pad 2, 256 bins).\\ \hline
...
The outputs serve three roles. Deployment consumes only the per-frame
\code{mask} (1 = reject, 0 = keep); in the proposed Kotekan interface, this
bit is the entire queue payload. The science-product path consumes the exact
\code{uint64} powers --- the archive pipeline forms the norm-corrected
decision on the host and stores the statistic --- and the exact \code{int32}
row sums, the input to the fine spectrum. The float ratio is a debugging
diagnostic, and the power readbacks double as cross-checks; none of these
cross the deployment interface.
...
\begin{itemize}
  ...
  \item Fine products require a library that exposes the row-sum entry. \code{fine_products=auto} degrades with \field{fine_status=kernel_library_lacks_matched_filter_row_projections}; \code{fine_products=on} hard-fails on a stale library, which is the intended production behavior.
  \item On DTV-occupied channels, a real pilot at a nonzero frequency offset occupies non-designated fine bins; an elevated non-designated flag rate can therefore be signal rather than CFAR miscalibration. The stored fine spectrum resolves which.
