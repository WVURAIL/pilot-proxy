...
% Single-sourced numeric specifications (versions, geometry, mu0 range,
...
fine axis (\code{docs/DESIGN_DECISIONS.md}). Because \field{fstat_raw}
...
\begin{itemize}
  ...
  \item Fine products require a library that exposes the row-sum entry. \code{fine_products=auto} degrades with \field{fine_status=kernel_library_lacks_row_sums}; \code{fine_products=on} hard-fails on a stale library, which is the intended production behavior.
