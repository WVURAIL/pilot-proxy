...
\end{lstlisting}
This example uses a 16,384-sample frame because that is the checked-in reference test profile. In integration, the frame size is matched to the correlator's frame rather than chosen freely --- the kernel requires only a positive multiple of 128 --- and the CHIME F-engine upgrade target this repository designs for is \(2^{14}=16384\). It is not a statement about the currently deployed CHIME correlator. The software does not infer one universal frame size from HDF5 metadata: set \code{chime-run --frame-size-samples} from verified product information, and set the datatrawl instrument \code{nfft} used by \code{chime-scan}. Record the resolved value with each real-data product.
