...
The Data Sheet
and \code{docs/product_schema_v3.md} give the definitions and stored fields;
...
offset pilot lives in non-designated bins, so the live false-alarm monitor
\field{fine_nd_flag_rate} reads high there; that is signal, not CFAR
miscalibration. Judge CFAR calibration on channels without detected lines,
where the monitor should sit within a factor of a few of the configured
\(P_{FA}\).
...
\field{fine_nd_flag_rate} counts detections in non-designated bins. A real transmitter at a nonzero pilot offset occupies those bins persistently, so a high rate on a DTV-occupied channel is expected signal (see Section~\ref{sec:fineproducts}). Evaluate CFAR calibration on channels without detected lines instead.
