...
\field{fstat_raw} is persisted per frame, so a deployment (or a later
...
\field{fine_status=kernel_library_lacks_row_sums}; \code{on} hard-fails on a
...
if (FStat_Supports_RowSums()) {
    /* d_row_sums_out: device int32, batch * N * rows * 2 elements
       (term-major, stream-major rows, interleaved re/im). */
    FStat_Compute_RowSums_I32(h, host_weights, d_row_sums_out);
}
\end{lstlisting}
