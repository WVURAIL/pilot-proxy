...
\title{PilotProxy CUDA F-Statistic DTV Pilot Detector v0.2.0\\User Guide}
\author{West Virginia University}
\date{PilotProxy-UG001 v1.6 \\ July 7, 2026}
...
\vspace{0.12in}
{\fontsize{23}{26}\selectfont\bfseries CUDA F-Statistic DTV Pilot Detector v0.2.0}\par
\vspace{0.08in}
{\Large User Guide}\par
\vspace{0.35in}
{\large PilotProxy-UG001 v1.6}\par
{\large July 7, 2026}\par
\vfill
\begin{tabularx}{\linewidth}{|>{\bfseries}L{0.25\linewidth}|Y|}
\hline
Document scope & Installed-package use, source-tree CUDA build, ATSC waveform generation, validation, detection, and interpretation of fixed-point kernel outputs.\\ \hline
Package version & \code{pilot-proxy} v0.2.0.\\ \hline
...
July 7, 2026 & 1.6 & Rolled for the chime-scan archive workflow, docs build
target, and current CHIME/CANFAR references.\\ \hline
July 3, 2026 & 1.5 & Updated for \code{pilot-proxy} v0.2.0.  Installed-package \code{pilot-proxy} command forms throughout; \code{evaluate-snr --detector-backend cpu-reference} for GPU-free publication sweeps; installation model and limitations refreshed (packaged weight/config resources; CUDA kernel builds from the source tree).\\ \hline
...
Install the package into the working environment, editable from a clone or as a built wheel/sdist:
\begin{lstlisting}[style=shell]
python -m pip install -e .
pilot-proxy --help
\end{lstlisting}
Installed packages carry the reference weight bank and the \code{configs/} tree, so every CPU-side command in this guide runs from the \code{pilot-proxy} console script alone.  The CUDA kernel is the one component that is not packaged: build it from the cloned source tree with \code{make build-kernel} (see Build and Test) before running GPU-backed commands.
...
\begin{itemize}
  ...
  \item CUDA Python: used for CuPy, the CUDA shared library, and detector evaluation --- the repository virtual environment (README: Environment) with \code{numpy}, \code{pytest}, and \code{cupy-cuda12x}.
\end{itemize}

Install \code{pilot-proxy} into the CUDA environment and keep that environment active; every listing in this guide is then a plain \code{pilot-proxy} invocation.  GNU Radio never needs to be on the command line directly: \code{generate-atsc} and the GNU Radio noise source run it through \code{--gnuradio-python} (typically \code{/usr/bin/python3}).  On WSL the CUDA driver path is \code{/usr/lib/wsl/lib}.

On WSL, do not add the Python environment's \code{lib/} directory to
\code{LD_LIBRARY_PATH}.  It can make \code{/usr/bin/bash} load that environment's
...
Pass \code{--no-python-user-site} if user-site packages shadow the NumPy build used by GNU Radio.
...
\begin{lstlisting}[style=shell]
bash examples/quickstart.sh   % GPU arch auto-detected
\end{lstlisting}
...
Run ATSC generation; GNU Radio executes under the interpreter given by \code{--gnuradio-python}:
\begin{lstlisting}[style=shell]
...
Run the end-to-end validation evaluator (CUDA backend; WSL driver path shown):
\begin{lstlisting}[style=shell]
...
Publication sweeps do not require a GPU.  With \code{--detector-backend cpu-reference} the primary fields come from the validated exact-integer CPU reference (shared result builder with the kernel path, exact rational-half mask); each output row records its backend.  Tie a CPU-produced curve to the deployed kernel afterwards with a small same-seed GPU spot check (\code{docs/PUBLICATION_VALIDATION.md}, item 2):
\begin{lstlisting}[style=shell]
pilot-proxy evaluate-snr \
  --input-iq generated/atsc/atsc_8vsb_complex64.cfile \
  --physical-channel 14 \
...
pilot-proxy detect \
  ...
  --threshold-snr-shelf-db -26
\end{lstlisting}
The pilot identity is read from the \code{metadata.json} sidecar that
\code{quantize} writes next to the matrix; an explicit
\code{--physical-channel} or \code{--dtv-pilot-mhz} must agree with it, and
with neither a sidecar nor a flag, \code{detect} refuses rather than guess.
...
\begin{itemize}
  \item Synthetic validation is GNU Radio ATSC 1.0 only; no other broadcast standards are modeled.
  \item The CHIME real-data workflow (\code{chime-run}, \code{chime-scan}, the HDF5 baseband reader) ships in \code{pilot_proxy.chime} and is documented by \code{docs/CANFAR_RUNBOOK.md} and \code{docs/PUBLICATION_VALIDATION.md}, not by this guide.
  \item No telescope-specific stream map is validated beyond example JSON metadata.
  \item Thresholded operation applies to the standalone synthetic detector; the CHIME workflow uses the parameter-free norm-corrected mask ($F>\mu_0$) rather than a threshold derived from telescope null statistics.
  \item Receiver profiles are an integration contract, not a validated telescope pipeline.
  \item Installed packages carry the weight bank and \code{configs/} resources; the CUDA kernel itself is not packaged and builds from the cloned source tree (\code{make build-kernel}).
\end{itemize}
