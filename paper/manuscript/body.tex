...
(i) a quantized-weight local-reference power-ratio detector for the pilot, computed from
...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    operating rule & positive excess (mask $F > \hat\mu_0$); fixed-$\tau$
...
The internal \textit{pilot-proxy} archive runtime supplies storage-safe
streaming of baseband events from the CHIME archive \citep[captured by the
system described in][]{michilli2021baseband}, resumable scans with exact
(event, frame) identity, and inventory provenance. Its
\texttt{chime-survey}, \texttt{chime-scan}, and
\texttt{chime-control-scan} commands use Datatrail for CADC discovery and
authenticated file resolution. The package is open source with citation
metadata (see the Data Availability statement); the detector data sheet and
user guide are attached to its releases.
...
(sha256 \texttt{b0dce17a\ldots}; retained as the historical half-band bank
under \texttt{weights/legacy\_halfband/}). This supports CPU$\leftrightarrow$GPU
...
\begin{itemize}
  ...
  % DYLAN: confirm against empirical_zero_points.csv (triage E2).
...
\begin{table}
  ...
  \begin{tabular}{rrrcrr}
    \toprule
    ATSC ch & CHIME ch & $\Delta$ & clean null core? & kept (F band) & kept (3-arm) \\
    \midrule
...
emission. \needsdylan{cross-check tail frames against Pilot Proxy archive
metadata and CHIME telemetry (dropped-packet counts, gain states); reclassify or exclude
...
One capped channel adds a sampled loud interval: within the frozen
snapshot, ch27 reads 38 per cent in 2020 Q3 (37 per cent in the FRB
stratum) against $\le$1.3 per cent in every 2025--26 quarter, with the
interior unsampled under the depth cap. The survey extension, complete
for this channel at the time of writing (6824 events, 32\,166 frames,
evaluated against the unchanged snapshot calibration;
\texttt{ch27\_extension\_quarterly.csv}), resolves it: ch27 was
\emph{continuously} loud from 2018 Q4 through 2020 Q4 --- hi-rates
39--54 per cent in every quarter, with a simultaneous 33--46 per cent
low tail, the two-sided displacement signature of a strong narrowband
occupant --- then transitions somewhere in the archive's 2021--2022 Q3
coverage hole, and holds quiet ($\le$3.7 per cent, typically $<$1) in
all fifteen quarters since. The full-depth 2020 Q3 rate, 38.6 per
cent, reproduces the capped sample's 38.0. Notably, the current
census holds no ch27 transmitter within 193\,km --- consistent with a
dominant occupant that left the channel in the 2021--2022 window and
is absent from present-day records. Beyond these, no channel
outside the episodic set sustains a rate above 3 per cent for
consecutive quarters. These epoch effects carry into the channel
...
interior, resolved by the completed survey extension
(Section~\ref{sec:secular}), shows a continuously loud 2018--2020 era
with a two-sided displacement signature, a transition inside the
archive's 2021--2022 coverage hole, and fifteen consecutive quiet
quarters since 2022 Q4 --- so its first-adoption standing applies to
exposure from 2022 Q4 onward. Earlier archival data on both inherits
the per-epoch treatment of the episodic set below. Channels 20 and 22 sit one notch behind the eight on a
...
\textit{pilot-proxy} is open source
(\texttt{github.com/\allowbreak WVURAIL/\allowbreak pilot-proxy}); a tagged
release with a Zenodo DOI will accompany submission, including the detector
data sheet and user guide, cited as software per Force11/RASTI policy. The repository's
