...
\author{D.~Gormley (with analysis assistant)}
...
\begin{figure}[h]
\centering\includegraphics[width=0.95\textwidth]{figS_ch30_two_population}
...
\begin{figure}[h]
\centering\includegraphics[width=0.9\textwidth]{figS_ch24_vs_time}
...
\begin{figure}[h]
\centering\includegraphics[width=0.72\textwidth]{figS_t2_block_profile}
...
\begin{figure}[h]
\centering\includegraphics[width=0.97\textwidth]{figS_threshold_exchange}
...
The survey extension then rewrote one of these stories wholesale
(Fig.~\ref{f:ch27i}): the frozen snapshot knew ch27 only as ``38 per
cent in 2020 Q3, interior unsampled.'' At full depth (6824 events),
ch27 was \emph{continuously} loud from 2018 Q4 through 2020 Q4 ---
39--54 per cent every quarter, with a simultaneous 33--46 per cent low
tail, the two-sided displacement signature of a strong narrowband
occupant --- then transitioned inside the archive's 2021--2022
coverage hole and has been quiet for fifteen straight quarters. The
capped sample's lone loud month was the visible tip of a 2.2-year
era; the full-depth 2020 Q3 rate (38.6 per cent) reproduces the
capped estimate (38.0) exactly. And the census supplies the ending:
no ch27 transmitter exists within 193\,km in the June-2026 records
--- the dominant occupant departed and left no present-day trace.
Lesson for the dissertation: a depth cap doesn't just widen error
bars, it can turn an \emph{era} into an \emph{anecdote}.

\begin{figure}[h]
\centering\includegraphics[width=0.9\textwidth]{figS_ch27_interior}
\caption{ch27's interior, resolved by the survey extension: a
continuously loud 2018--2020 era displacing $F$ in both directions,
the transition hidden in the archive's coverage hole, and fifteen
quiet quarters since.}
\label{f:ch27i}
\end{figure}

\section{The one-stream night: how a silent default became a diagnosis}
A full evening went to a detector that read $P_d = 0.84$ where the
archive said 0.967 --- on two machines, two noise sources, and two
captures. Every environment hypothesis failed in turn (WSL, GNU
Radio, the file bytes, even the archived reference, which turned out
to predate the public repo import entirely). The eventual diagnosis
took three summary statistics: the mean response was healthy, the
decision boundary sat exactly at $\mu_0$, and the width was doubled
--- and $\sigma \propto 1/\sqrt{R}$ says doubled width means
one-quarter the rows. The repo import had changed
\texttt{--num-input-streams} from 4 to 1, silently, and every
qualification run had been measuring 128-row trials against a 512-row
reference. The reduced-variable framework diagnosed its own harness
bug from two numbers. Standing protocol since: every sweep command
pins its dimensionality and noise source explicitly, every new host
runs a 100-trial anchor smoke before bulk work, and defaults are
never trusted across a migration boundary. (The corollary finding:
with the python noise source the entire float front end is
bit-reproducible across WSL and CANFAR --- the environment was never
the problem; the configuration was.)
...
\begin{figure}[h]
\centering\includegraphics[width=\textwidth]{figS_tone_gallery}
...
\begin{figure}[p]
\centering\includegraphics[width=0.94\textwidth]{fig_spectra_all23_fullspan_before}
...
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.94\textwidth]{fig_spectra_all23_fullspan_instrument_removed}
...
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.94\textwidth]{fig_spectra_all23_fullspan_after}
\caption{Same, after the analytic positive-excess production mask ---
what the detector actually removes.}
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.94\textwidth]{fig_spectra_all23_pilot_zoom_instrument_removed}
...
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.94\textwidth]{fig_excess_threshold_all23}
...
\end{figure}
\begin{figure}[p]
\centering\includegraphics[width=0.86\textwidth]{fig_diurnal_mask_fraction}
