...
def evaluate_frame_health(product: Mapping[str, Any]) -> FrameHealthResult:
    ...
    coarse = np.asarray(
        _frame_vector(product, ARCHIVED_COARSE_POWER_RATIO, n_frames),
        dtype=np.float64,
    )
    fine = _array(product, ARCHIVED_FINE_POWER_RATIO)
    if fine.ndim != 2 or fine.shape[0] != n_frames or fine.shape[1] <= 0:
        raise ArchiveHealthError(
            f"{ARCHIVED_FINE_POWER_RATIO} must have shape (N, B), B > 0; got {fine.shape}"
        )
    ...
    reasons: dict[str, np.ndarray] = {
        ...
        REASON_FINE_NONFINITE: valid & ~np.all(np.isfinite(fine), axis=1),
        REASON_FINE_NEGATIVE: valid & np.any(fine < 0.0, axis=1),
    }
...
def write_baonoise_health_view(source: Path, destination: Path) -> Path:
    ...
    with np.load(source_path, allow_pickle=False) as product:
        ...
        for name in _BAONOISE_FRAME_FIELDS:
            ...
            arrays[name] = np.asarray(value[health.include])
        for name in _BAONOISE_METADATA_FIELDS:
            arrays[name] = np.asarray(_array(product, name))
...
def recompute_corrected_fine_diagnostics(
    ...
    *,
    ...
    fine = np.asarray(_array(product, ARCHIVED_FINE_POWER_RATIO), dtype=np.float64)
    n_frames, n_bins = fine.shape
        ...
        summary = {
            ...
            "scalar_distributions_health_included": {
                ...
                "pilot_excess_db": distribution_summary(
                    _frame_vector(product, ARCHIVED_PILOT_EXCESS_DB, n_frames)[include]
                ),
                "estimated_data_shelf_snr_db": distribution_summary(
                    _frame_vector(product, ARCHIVED_DATA_SHELF_SNR_DB, n_frames)[include]
                ),
    ...
    *,
    ...
    with np.load(product_path, allow_pickle=False) as product:
        ...
        panels = [
            ...
            (
                _frame_vector(
                    product, ARCHIVED_NORMALIZED_COARSE_POWER_RATIO_DB, n_frames
                )[include],
