...
def _clean_packed_terms(
    ...
    *,
    ...
    weights_path = Path(DEFAULT_WEIGHTS_PATH).resolve()
...
def _bootstrap_pooled_excess_interval(
    ...
    *,
    ...
    pairs = _trial_ratio_weight_pairs(rows, prefix)
    if len(pairs) < 2 or int(samples) < 1:
        return math.nan, math.nan
    ratios = np.asarray([pair[0] for pair in pairs], dtype=np.float64)
    ...
    pooled = np.sum(ratios[indices] * sampled_weights, axis=1) / np.sum(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Trial bootstrap resamples; zero disables intervals.",
    )
    parser.add_argument(
        "--y-min-db",
        type=float,
