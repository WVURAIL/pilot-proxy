...
def test_chime_runner_writes_frequency_offset_products_in_same_run(tmp_path) -> None:
    input_dir = tmp_path / "input"
    ch_dir = input_dir / "ch0844"
    ch_dir.mkdir(parents=True)
    physical_channel = 14
    ...
    with h5py.File(ch_dir / "001.h5", "w") as h5:
        ...
        h5.attrs["freq_id"] = 844
        h5.attrs["physical_channel"] = physical_channel
        ds = h5.create_dataset("baseband", data=streams.T)
        ds.attrs["axis"] = np.asarray(["time", "input"], dtype=object)
    ...
    outputs = run_chime_analysis(
        ...
        frequency_offset_peak_search_half_width_hz=2_000.0,
        frequency_offset_backend="numpy",
        kernel=_fake_kernel(128),
        detector_fn=fake_detector,
        weights_by_channel={physical_channel: np.ones((3, 128), dtype=np.int8)},
    )

    assert outputs["frequency_offset_outputs"].exists()
    assert outputs["frequency_offset_summary"].exists()
    ...
    assert float(np.nanmedian(products["frequency_offset_hz"])) == pytest.approx(
        true_error_hz,
        abs=sample_rate_hz / frame_size,
    )
