...
def _product(
    *,
    ...
    return {
        ...
        "mu0": np.asarray([1.0]),
        # The correction is derived from these; mu0 above is the
        # archived float the migration still accepts.
        "target_norm_sq": np.asarray([1], dtype=np.int64),
        ARCHIVED_REFERENCE_NORM_SUM_SQ: np.asarray([2], dtype=np.int64),
