...
def current_product() -> dict[str, np.ndarray]:
    return {
        ...
        "fstat_raw": np.asarray([[1.0]]),
        "fstat_level_db": np.asarray([[0.0]]),
        "pnr_bin_db": np.asarray([[np.nan]]),
        "snr_shelf_db": np.asarray([[np.nan]]),
        ...
        "mu0": np.asarray([1.0]),
        "pilot_excess_corrected": np.asarray([[0.0]]),
        ...
        "fstat_fine": np.zeros((1, 0), dtype=np.float32),
