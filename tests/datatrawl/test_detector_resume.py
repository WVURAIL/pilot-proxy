...
def test_resume_refuses_fine_definition_change_end_to_end(tmp_path, monkeypatch):
    ...
    import pilot_proxy.datatrawl_plugins.detector as det_mod
...
def test_fine_detected_frame_indices_are_global(tmp_path):
    ...
    a = PilotProxyDetectorAnalyzer()
    for i in range(2):
        meta = dict(reader.probe(str(files[(f"evt{i}", FREQ_ID)])))
        meta["unit_key"] = f"synth:{i}"
        if i == 0:
            a.begin(ctx, meta)
        a.consume_file(reader.iter_arrays(str(files[(f"evt{i}", FREQ_ID)]), ctx), meta)
