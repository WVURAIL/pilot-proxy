...
HEADERS = REPO.parent/"results/calibration_progress_2026-09-09/sdr/smoke-build-v2/include"
DRIVER = runpy.run_path(str(REPO/"tools/lime_smoke_capture_v2.py"))
FAKE = r"""
...
def fake_build(tmp_path_factory):
    ...
    subprocess.run(["g++", "-std=c++17", "-pthread", "-I", str(HEADERS), str(REPO/"tools/lime_smoke_worker_v2.cpp"), str(library), "-o", str(worker)], check=True)
