...
static std::atomic<bool> tx_activated{false};
...
int LMS_RecvStream(lms_stream_t*,void* output,size_t n,lms_stream_meta_t* m,unsigned){
 ...
 if(mode("late_gap") && tx_activated.load())m->timestamp+=samples/16384;
...
int LMS_GetStreamStatus(lms_stream_t* stream,lms_stream_status_t* s){
 ...
 if(!stream->isTx){
  ...
  if(mode("late_drop") && tx_activated.load())s->droppedPackets=1;
 }
 return 0;
}
}
...
def fake_build(tmp_path_factory):
    ...
    subprocess.run(["g++", "-std=c++17", "-pthread", "-I", str(HEADERS), str(REPO/"tools/lime_smoke_worker_v3.cpp"), str(library), "-o", str(worker)], check=True)
...
@pytest.mark.parametrize("mode,tx_attempted", [("no_rx", False), ("gap", False), ("clip", False), ("tx_fail", True), ("antenna_fail", True), ("gain_mismatch", False)])
...
def test_startup_budget_or_post_ready_errors_are_fatal(tmp_path, fake_build, monkeypatch, mode):
    ...
    if mode=="always_drop":
        assert not report["tx_attempted"] and report["captured_samples"] == 0
