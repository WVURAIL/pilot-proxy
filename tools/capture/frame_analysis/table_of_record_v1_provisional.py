...
usage: table_of_record.py <out csv> <dump label>=<frame_residual csv>:<chime-run dir> [more dumps ...]
...
for arg in sys.argv[2:]:
    label, rest = arg.split("=", 1); fr_csv, run_dir = rest.split(":", 1)
...
with open(sys.argv[1], "w") as fh:
    w = csv.writer(fh); w.writerow(["freq_id", "channel", "role", "disposition", "policy_or_reason", "A_keep_all_ns1_bound", "A_keep_all_class_median", "G_dump", "lambda_none", "lambda_deployed", "suppression_db", "R_none_G1", "R_deployed_G1", "R_none_Gdump", "R_deployed_Gdump", "G_archive", "R_deployed_Garchive", "kept_dumps_cal_q0.5", "kept_dumps_cal_q0.9", "n_dumps"])
...
print(f"wrote {sys.argv[1]} with {845-477} rows from {len(dumps)} dump(s): {', '.join(dumps)}")
