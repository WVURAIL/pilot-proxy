...
for the live frequency count, df -h /data on outrigger-buffer for the free space). --work-only re-deposits
...
p.add_argument("--free-gb", type=float, required=True, help="free GB on outrigger-buffer:/data (runbook step 1)")
p.add_argument("--dm", type=float, default=0.0)
p.add_argument("--user", default="dgormley")
p.add_argument("--master", default="http://csBfs:54323/baseband")
...
try:
    ...
    print(f"    check:   curl -s 'http://csBfs:54323/baseband-status?event_id={event_id}'")
...
print(f"done. Watch: curl -s 'http://csBfs:54323/baseband-status?event_id={event_id}'")
