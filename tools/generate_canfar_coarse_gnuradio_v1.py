...
def prepare(output,frames,evidence_dir=None):
    ...
    plan={"schema":"canfar-coarse-gnuradio-plan-v1","created_utc":datetime.now(timezone.utc).isoformat(),
          ...
          "actual_gnuradio_blocks":["analog.noise_source_c(GR_GAUSSIAN,1,seed) x3","analog.sig_source_c(GR_CONST_WAVE,sqrt(gamma))","blocks.add_cc","blocks.complex_to_mag_squared x3","blocks.head x3","custom gr.sync_decimator with np.sum(dtype=float64)","blocks.file_descriptor_sink"],
...
def generate(output,state):
    ...
    if plan.get("schema")!="canfar-coarse-gnuradio-plan-v1" or sha(output/"plan.json")!=json.loads((output/"plan-digest.json").read_text())["sha256"]:
    ...
    class PowerSums(gr.sync_decimator):
        def __init__(self):
            gr.sync_decimator.__init__(self,name="float64 independent branch power sums",in_sig=[np.float32]*3,out_sig=[(np.float64,4)],decim=P)
        def work(self,input_items,output_items):
            ...
            return rows
    receipt={"schema":"canfar-coarse-gnuradio-receipt-v1","state":state,"success":False,
