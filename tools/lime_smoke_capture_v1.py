...
def prepare(output, *, build_manifest, serial, frequency_hz, rate_hz=2000000., native_rx_gain_db=30):
    ...
    command = [str(worker), "--serial", serial, "--frequency-hz", str(frequency_hz),
               ...
               "--native-rx-gain-db", str(native_rx_gain_db), "--tx-samples", str(waveform.size),
               "--tx-file", str(tx_path), "--rx-file", str(output/"rx.cfile"),
               "--status-file", str(output/"worker-status.json")]
    plan = {"schema": "lime-confined-smoke-plan-v1", "frozen_utc": now(),
            ...
            "native_tx_gain_db": 0, "native_rx_gain_db": native_rx_gain_db,
...
def capture(output, *, transmit=False, rf_confined_authorized=False):
    ...
    receipt = {"schema": "lime-confined-smoke-receipt-v1", "hardware_attempted": True,
    ...
    try:
        ...
        receipt["success"] = bool(receipt.get("returncode") == 0 and not timeout and not requested_stop
                                  and receipt["cleanup_confirmed"] and receipt.get("worker_report", {}).get("success"))
...
def main():
    ...
    if args.stage == "prepare":
        if args.build_manifest is None or args.serial is None or args.frequency_hz is None:
            parser.error("prepare needs build manifest, explicit serial and frequency")
        result = prepare(args.output, build_manifest=args.build_manifest, serial=args.serial,
                         frequency_hz=args.frequency_hz, native_rx_gain_db=args.native_rx_gain_db)
    else:
