...
def prepare(output, *, build_manifest, serial, frequency_hz, expected_tx_gain_readback_db, rate_hz=2000000., native_rx_gain_db=30):
    ...
    if type(expected_tx_gain_readback_db) is not int or not 0 <= expected_tx_gain_readback_db <= 12:
        raise ValueError("explicit expected TX readback must be an integer from 0 through 12")
    ...
    command = [str(worker), "--serial", serial, "--frequency-hz", str(frequency_hz),
               ...
               "--native-rx-gain-db", str(native_rx_gain_db),
    ...
    plan = {"schema": "lime-confined-smoke-plan-v2", "frozen_utc": now(),
            "scope": "uncalibrated single-RF-path tone transport, no measured dBm or scientific calibration",
            "hardware_attempted": False, "library": str(library), "command": command,
            "serial": serial, "frequency_hz": frequency_hz, "sample_rate_hz": rate_hz,
            "native_tx_gain_db": 0, "requested_native_tx_gain_db": 0,
            ...
            "submitted_samples": int(waveform.size), "measured_power_dbm": None,
...
def capture(output, *, transmit=False, rf_confined_authorized=False):
    ...
    receipt = {"schema": "lime-confined-smoke-receipt-v2", "hardware_attempted": True,
...
def main():
    ...
    if args.stage == "prepare":
        if args.build_manifest is None or args.serial is None or args.frequency_hz is None or args.expected_tx_gain_readback_db is None:
            parser.error("prepare needs build manifest, explicit serial, frequency and expected TX gain readback")
        result = prepare(args.output, build_manifest=args.build_manifest, serial=args.serial,
                         frequency_hz=args.frequency_hz, expected_tx_gain_readback_db=args.expected_tx_gain_readback_db,
