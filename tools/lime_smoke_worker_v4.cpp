...
struct Options {
    std::string serial, tx_file, rx_file, status_file, startup_file, chunks_file, tx_status_file;
...
Options parse(int argc, char** argv) {
    ...
    require(values.size() == 14, "Unknown argument");
...
int run(const Options& o) {
    ...
    Output rx_output(o.rx_file), status_output(o.status_file), startup_output(o.startup_file), chunks_output(o.chunks_file), tx_status_output(o.tx_status_file);
    ...
    bool tx_attempted=false, device_opened=false, tx_started_before_qualification=false, tone_submission_attempted=false, tx_final_status_saved=false;
    ...
    auto sample_tx_status=[&](const std::string& phase, bool require_active=true) {
        ...
        if(ret==0) {
            ...
            auto& portion=tone_submission_attempted?tx_after_submission:tx_before_submission;
        ...
        row<<"{\"query_index\":"<<tx_status_queries++<<",\"phase\":"<<quoted(phase)<<",\"status_return\":"<<ret
           <<",\"tone_submission_attempted\":"<<(tone_submission_attempted?"true":"false")<<",\"samples_accepted_so_far\":"<<sent
    ...
    try {
        ...
        while (sent<o.tx_samples) {
            ...
            tone_submission_attempted=true;
        ...
        sample_tx_status("after_capture");tx_final_status_saved=true;
