...
def _load_result_rows(path: Path) -> dict[int, dict[str, Any]]:
    ...
    for index, line in enumerate(lines):
        try:
            ...
            if index == len(lines) - 1:
                break
            raise
        rows[int(row["event"]["event_index"])] = row
    return rows
...
def run(
    ...
    *,
    ...
    if manifest_path.exists():
        ...
        if _stable_manifest(existing_manifest) != _stable_manifest(expected_manifest):
    ...
    rows_by_event = _load_result_rows(results_path) if args.resume else {}
